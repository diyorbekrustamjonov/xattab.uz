...
export default function SharePost({ title, slug, description }: props) {
  const blog = encodeURIComponent('https://xattab.uz/blog/');
  const options = [
    {
      icon: BiLogoTwitter,
      name: "Twitter",
      shareUrl: `https://twitter.com/intent/tweet?text=${encodeURIComponent('Thank you @xattab for writing this post.')}.%0A%0A${title}%0A%0A${blog}${slug}`,
    },
    {
    ...
    },
    {
      icon: BiLogoWhatsapp,
      name: 'WhatsApp',
      shareUrl: `https://api.whatsapp.com/send?text=${encodeURIComponent('Read this amazing article by Diyorbek Rustamjonov')}.%0A%0A${title}%0A%0A${blog}${slug}`
    },
  ];
